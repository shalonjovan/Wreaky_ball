...
function gameLoop(timestamp) {
  ...
  if (timestamp - lastEnemySpawn > enemySpawnInterval) {
    ...
    const gameTime = (timestamp - gameStartTime) / 1000; // Convert to seconds
    const targetTime = 105; // 1.75 minutes
    const progressRatio = Math.min(gameTime / targetTime, 1); // Cap at 1.0
